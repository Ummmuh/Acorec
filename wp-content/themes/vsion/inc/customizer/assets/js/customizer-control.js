...
( function( $ ) {
	...
	VSION_Customizer = {
		...
		_initToggles: function()
		{
			...
			VSION_ControlTrigger.triggerHook( 'vsion-toggle-control', api );

			// Loop through each setting.
			$.each( VSION_CustomizerToggles, function( settingId, toggles ) {
						...
						$.each(toggles, function( i, toggle) {
							...
							$.each(toggle.controls, function( k, controlId) {
								...
								if ( typeof parent != 'undefined' ) {
									...
									if ( ! visibleIt ) {
										$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'vsion-hide' );
									} else {
										$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'vsion-hide' );
									}
								}
							});
						});
	...
	$( function() { VSION_Customizer.init(); } );
