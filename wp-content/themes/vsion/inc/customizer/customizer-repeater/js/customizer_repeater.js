/* global jQuery */
/* global wp */
function vsion_media_upload(button_class) {
    'use strict';
 ...
 *** Generate unique id ***
 *********************************************/
function vsion_customizer_repeater_uniqid(prefix, more_entropy) {
    'use strict';
 ...
 *** General Repeater ***
 *********************************************/
function vsion_customizer_repeater_refresh_social_icons(th) {
    'use strict';
    ...
    th.find('.customizer-repeater-social-repeater-container').each(function () {
        ...
        if (!id) {
            id = 'customizer-repeater-social-repeater-' + vsion_customizer_repeater_uniqid();
    ...
    vsion_customizer_repeater_refresh_general_control_values();
}


function vsion_customizer_repeater_refresh_general_control_values() {
    'use strict';
    jQuery('.customizer-repeater-general-control-repeater').each(function () {
        ...
        th.find('.customizer-repeater-general-control-repeater-container').each(function () {
            ...
            if (!id) {
                id = 'social-repeater-' + vsion_customizer_repeater_uniqid();
            ...
            if (text !== '' || image_url !== '' || image_url2 !== '' || title !== '' || open_new_tab !== '' || link_follow_nofollow !== '' || subtitle !== '' || icon_value !== '' || link !== '' || choice !== '' || social_repeater !== '' || shortcode !== '' || slide_align !== '' || color !== '') {
                values.push({
                    ...
                    'color': color,
                    'color2': color2,
                    'text': vsion_escapeHtml(text),
                    'link': link,
                    'text2': vsion_escapeHtml(text2),
					'button_second': vsion_escapeHtml(button_second),
                    'link2': link2,
                    'image_url': (choice === 'customizer_repeater_none' ? '' : image_url),
                    'choice': choice,
                    'image_url2': image_url2,
                    'title': vsion_escapeHtml(title),
                    'subtitle': vsion_escapeHtml(subtitle),
					'subtitle2': vsion_escapeHtml(subtitle2),
					'subtitle3': vsion_escapeHtml(subtitle3),
					'subtitle4': vsion_escapeHtml(subtitle4),
					'subtitle5': vsion_escapeHtml(subtitle5),
					'slide_align': vsion_escapeHtml(slide_align),
					'link_follow_nofollow': vsion_escapeHtml(link_follow_nofollow),
					'open_new_tab' : open_new_tab,
                    'social_repeater': vsion_escapeHtml(social_repeater),
                    'id': id,
                    'shortcode': vsion_escapeHtml(shortcode)
                });
            }
...
jQuery(document).ready(function () {
    'use strict';
    var vsion_theme_controls = jQuery('#customize-theme-controls');
    vsion_theme_controls.on('click', '.customizer-repeater-customize-control-title', function () {
    ...
    vsion_theme_controls.on('change', '.icp',function(){
        vsion_customizer_repeater_refresh_general_control_values();
        return false;
    });
	
	vsion_theme_controls.on('change','.customizer-repeater-slide-align', function(){
		vsion_customizer_repeater_refresh_general_control_values();
        return false;
		
	});
	
	vsion_theme_controls.on('change','.customizer-repeater-follow-nofollow', function(){
		vsion_customizer_repeater_refresh_general_control_values();
        return false;
		
	});

    vsion_theme_controls.on('change','.customizer-repeater-image2-control', function(){
        vsion_customizer_repeater_refresh_general_control_values();
        return false;
        
    });
	
    vsion_theme_controls.on('change', '.customizer-repeater-image-choice', function () {
    ...
    vsion_media_upload('.customizer-repeater-custom-media-button');
     ...
     *
     */
    vsion_theme_controls.on('click', '.customizer-repeater-new-field', function () {
        var split_add_more_button=jQuery(this).text();
        var split_add_more_button_split=split_add_more_button.substr(4, 12);
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-' + vsion_customizer_repeater_uniqid();
		
		if(split_add_more_button_split=="Add New Slid")
        {
        if(jQuery('#exist_vsion_Slide').val()>=3)
        {
		...
		alert(split_add_more_button_split);
         var new_service_add_val=parseInt(jQuery('#exist_vsion_Slide').val())+1;
         jQuery('#exist_vsion_Slide').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Serv")
        {
        if(jQuery('#exist_vsion_Service').val()>=4)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_vsion_Service').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_vsion_Service').val())+1;
         jQuery('#exist_vsion_Service').val(new_service_add_val);  
        }
        }
        if(split_add_more_button_split=="Add New Funf")
        {
        if(jQuery('#exist_vsion_Funfact').val()>=4)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_vsion_Funfact').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_vsion_Funfact').val())+1;
         jQuery('#exist_vsion_Funfact').val(new_service_add_val);  
        }
        }
		if(split_add_more_button_split=="Add New Feat")
        {
        if(jQuery('#exist_vsion_Feature').val()>=6)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_vsion_Feature').val()<6)
        {
         var new_service_add_val=parseInt(jQuery('#exist_vsion_Feature').val())+1;
         jQuery('#exist_vsion_Feature').val(new_service_add_val);  
        }
        }
		if(split_add_more_button_split=="Add new fiel")
        {
        if(jQuery('#exist_vsion_Icons').val()>=5)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_vsion_Icons').val()<5)
        {
         var new_service_add_val=parseInt(jQuery('#exist_vsion_Icons').val())+1;
         jQuery('#exist_vsion_Icons').val(new_service_add_val);  
        }
        }
    ...
    vsion_theme_controls.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        if (typeof    jQuery(this).parent() !== 'undefined') {
            jQuery(this).parent().hide(500, function(){
                ...
                {
                jQuery('#exist_vsion_Slide').val(parseInt(jQuery('#exist_vsion_Slide').val())-1);  
                }
				if(split_delete_button_split=="Delete Servi")
                {
                jQuery('#exist_vsion_Service').val(parseInt(jQuery('#exist_vsion_Service').val())-1);  
                }
				if(split_delete_button_split=="Delete Funfact")
                {
                jQuery('#exist_vsion_Funfact').val(parseInt(jQuery('#exist_vsion_Funfact').val())-1); 
                }
				if(split_delete_button_split=="Delete Icons")
                {
                jQuery('#exist_vsion_Icons').val(parseInt(jQuery('#exist_vsion_Icons').val())-1); 
                }
            });
        }
        return false;
    ...
    vsion_theme_controls.on('keyup', '.customizer-repeater-title-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });
    ...
    vsion_theme_controls.on('keyup', '.customizer-repeater-subtitle-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });
	
	 vsion_theme_controls.on('keyup', '.customizer-repeater-subtitle2-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });
	
	vsion_theme_controls.on('keyup', '.customizer-repeater-subtitle3-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });
	
	vsion_theme_controls.on('keyup', '.customizer-repeater-subtitle4-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });
	
	vsion_theme_controls.on('keyup', '.customizer-repeater-subtitle5-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });

    vsion_theme_controls.on('keyup', '.customizer-repeater-shortcode-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });

    vsion_theme_controls.on('keyup', '.customizer-repeater-text-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });

    vsion_theme_controls.on('keyup', '.customizer-repeater-link-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });

    vsion_theme_controls.on('keyup', '.customizer-repeater-text2-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });
	
	vsion_theme_controls.on('keyup', '.customizer-repeater-button2-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });

    vsion_theme_controls.on('keyup', '.customizer-repeater-link2-control', function () {
        vsion_customizer_repeater_refresh_general_control_values();
    });
	
	/* vsion_theme_controls.on('change','.customizer-repeater-checkbox', function(){
    ...
    /*----------------- Socials Repeater ---------------------*/
    vsion_theme_controls.on('click', '.social-repeater-add-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-social-repeater-' + vsion_customizer_repeater_uniqid();
        if (typeof th !== 'undefined') {
    ...
    vsion_theme_controls.on('click', '.social-repeater-remove-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var repeater = jQuery(this).parent().parent();
        th.remove();
        vsion_customizer_repeater_refresh_social_icons(repeater);
        return false;
    });

    vsion_theme_controls.on('keyup', '.customizer-repeater-social-repeater-link', function (event) {
        event.preventDefault();
        var repeater = jQuery(this).parent().parent();
        vsion_customizer_repeater_refresh_social_icons(repeater);
        return false;
    ...
    vsion_theme_controls.on('change', '.customizer-repeater-social-repeater-container .icp', function (event) {
        event.preventDefault();
...
function vsion_escapeHtml(string) {
    'use strict';
    //noinspection JSUnresolvedFunction
    string = String(string).replace(new RegExp('\r?\n', 'g'), '<br />');
    string = String(string).replace(/\\/g, '&#92;');
    return String(string).replace(/[&<>"'\/]/g, function (s) {
        return vsion_entityMap[s];
    });
